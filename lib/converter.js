  ...
  for (i = 0; i < ast.length; i++) {
    ...
    if (item.type === 'heading') {
      if (item.depth === 1) {
        ...
        if ((item.depth === 3 || item.depth === 4)&& ast[i + 1].type === 'code' && (!ast[i + 1].lang || CODE_BLOCK_LANGS.indexOf(ast[i + 1].lang.toLowerCase()) !== -1)) {
    ...
    else if (item.type === 'paragraph' && item.text.indexOf('__') === -1) {
